export function camelToTitle(str: string) {
  return dasherize((str || '')).split('-').map((str: string) => {
    return ucFirst(str);
  }).join(' ');
}
...
const isKeyOfEntityMap = (input: string): input is keyof EntityMap => Object.keys(entityMap).includes(input);

export function escapeHtml(html: any) {
  return String(html).replace(/[&<>"']/g, (s: string) => {
    if (!isKeyOfEntityMap(s)) {
      return s;
    }
...
export function random32(count: number) {
...
export function randomStr(length = 16, chars = CHARSET.ALPHA_NUM) {
  ...
  return random32(length).map((val: any) => {
    return chars[val % chars.length];
  }).join('');
}

export function formatPercent(value: number, maxPrecision = 2) {
...
export function resourceNames(names: any, plusMore: any, t: any) {
  return names.reduce((res: any, name: any, i: any) => {
    if (i >= 5) {
      return res;
    }
...
export function indent(lines: any, count = 2, token = ' ', afterRegex = null) {
...
export function asciiLike(str: string) {
  str = str || '';
...
export function coerceStringTypeToScalarType(val: any, type: any) {
  if ( type === 'float' ) {
...
export function matchesSomeRegex(stringRaw: string, regexes = []) {
...
export function ensureRegex(strOrRegex: any, exact = true) {
...
export function splitObjectPath(path: any) {
  if ( path.includes('"') || path.includes("'") ) {
    // Path with quoted section
    return path.match(quotedMatch).map((x: any) => x.replace(/['"]/g, ''));
  }
...
export function joinObjectPath(ary: any) {
  let out = '';
...
export function shortenedImage(image: any) {
  return (image || '')
