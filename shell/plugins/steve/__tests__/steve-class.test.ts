          ...
          it.each([
            {
              ...
              labelForResult:    'CustomResourceDefinition',
            ...
            {
              name:     'using translation fallback when labelFor returns undefined',
              response: {
                _status:  201,
                metadata: { generateName: 'test-generated-' },
                id:       'default/test-generated-abc123'
              },
              labelForResult:    undefined,
              translationResult: 'Custom Resource Definitions',
              kind:              undefined,
              expectedResource:  'Custom Resource Definitions',
              expectedId:        'test-generated-abc123'
            },
            {
              name:     'using kind fallback when labelFor and translation return undefined',
              response: {
                _status:  201,
                metadata: { generateName: 'test-generated-' },
                id:       'default/test-generated-abc123'
            ...
            response, labelForResult, translationResult, kind, expectedResource, expectedId
          }) => {
            const mockDispatch = jest.fn();
            const mockT = jest.fn((key, params) => {
              if (key === `typeLabel."${ customResource.type }"`) {
            ...
            const mockSchema = { linkFor: jest.fn() };
            const mockLabelFor = jest.fn().mockReturnValue(labelForResult);
            const mockRootGetters = {
              'i18n/t':            mockT,
              'type-map/labelFor': mockLabelFor
            };
            const resource = kind ? { ...customResource, kind } : customResource;
            const steve = new Steve(resource, {
              getters:     { schemaFor: () => mockSchema },
              dispatch:    mockDispatch,
              rootGetters: mockRootGetters,
            });
            ...
            expect(mockLabelFor).toHaveBeenCalledWith(mockSchema);
            ...
            // Check if translation fallback was called (when labelFor returned undefined, we call typeLabel translation)
            expect(mockT).toHaveBeenCalledTimes(labelForResult ? 2 : 3);
