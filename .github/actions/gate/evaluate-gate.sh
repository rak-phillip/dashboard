...
#
# Why this exists: GitHub evaluates required status checks twice under a merge
# queue — on the pull request to enter, and on the merge group to merge — using
# the same context names, and a `skipped` job is NOT a success for a required
# check. Aggregating the expensive PR-only jobs behind one always-running job is
# what allows the merge queue to run a reduced check set without stalling.
...
done < <(jq -r 'to_entries[] | "\(.key) \(.value.result // "missing")"' <<<"$NEEDS")
